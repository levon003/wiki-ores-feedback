    ...
    with open(output_filepath, 'w') as outfile:
        for token, count in counter.most_common():
            if token == '\n':
                ...
                token = f'WHITESPACE{whitespace_counter}'
                whitespace_counter += 1
...
def build_counters(diff_json_filepath, output_dir, rev_id_is_reverted_dict):
    ...
    save_counter(content_counter, os.path.join(output_dir, 'all_content_counts.csv'))
    save_counter(removed_counter, os.path.join(output_dir, 'all_removed_counts.csv'))
    save_counter(inserted_counter, os.path.join(output_dir, 'all_inserted_counts.csv'))
    save_counter(content_oc, os.path.join(output_dir, 'all_content_doc_counts.csv'))
    save_counter(removed_oc, os.path.join(output_dir, 'all_removed_doc_counts.csv'))
    save_counter(inserted_oc, os.path.join(output_dir, 'all_inserted_doc_counts.csv'))
    save_counter(reverted_content_oc, os.path.join(output_dir, 'reverted_content_doc_counts.csv'))
    save_counter(reverted_removed_oc, os.path.join(output_dir, 'reverted_removed_doc_counts.csv'))
    save_counter(reverted_inserted_oc, os.path.join(output_dir, 'reverted_inserted_doc_counts.csv'))
