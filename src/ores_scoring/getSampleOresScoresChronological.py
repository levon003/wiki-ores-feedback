...
def main():
    ...
    print(f"Found {len(ores_df)} existing ORES scores.")
    
    # remove scored revs from the rev_df
    revs_with_ores_scores = set(ores_df.rev_id)
    rev_df = rev_df[~rev_df.rev_id.isin(revs_with_ores_scores)]
    ...
    with open(ores_score_filepath, 'a') as outfile:
        total_processed = 0
