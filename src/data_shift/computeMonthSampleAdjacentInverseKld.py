...
def main():
    ...
    with Pool(processes=32) as pool:
        multiple_results = [pool.apply_async(compute_kld, kwds={'sdf': month_feature_df_list[i+1].drop(columns='damaging'), 
                                                                'tdf': month_feature_df_list[i].drop(columns='damaging')}) 
    ...
    output_filepath = os.path.join(working_dir, 'month_sample_adjacent_inverse_kld_200.csv')
